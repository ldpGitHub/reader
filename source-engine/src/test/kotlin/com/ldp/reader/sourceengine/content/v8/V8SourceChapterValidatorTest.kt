...
class V8SourceChapterValidatorTest {
    ...
    @Test
    fun reportsEarliestBadTailEvenWhenLaterSampleIsNormal() {
        val result = V8SourceRunResult(
            title = "尾部探针",
            author = "test",
            sourceKey = "source-a",
            marks = listOf(
                mark(762, V8ChapterMarkState.NORMAL),
                mark(763, V8ChapterMarkState.WRONG),
                mark(764, V8ChapterMarkState.WRONG),
                mark(765, V8ChapterMarkState.NORMAL)
            )
        )
    ...
    @Test
    fun futureRescueCannotOpenNormalHoleAfterBadTail() {
        val validator = V8SourceChapterValidator(
            semanticModel = TailRescueSemanticModel(),
            config = V8PsbmtConfig(
                minLowThreshold = 0.015,
                maxLowThreshold = 0.055,
                lowThresholdMinDrop = 0.02,
        ...
        val result = validator.validate(
            V8SourceRunRequest(
                ...
                chapters = listOf(
                    ...
                    V8ChapterInput(4, "第五章", taggedForeign("CURRENT_FOREIGN")),
                    V8ChapterInput(5, "第六章", taggedForeign("FUTURE_RESCUE_FOREIGN"))
                ),
        ...
        assertEquals(brief, V8ChapterMarkState.WRONG, result.marks[1].state)
        assertTrue(result.marks[1].reasons.any { reason -> reason == "v8 tailContinuationAfterBadTail=true" })
    }
    ...
    private fun taggedForeign(tag: String): String {
        return buildString {
            repeat(18) { round ->
                append(tag).append(' ')
                append("韩砚站在太空港候机厅，透明穹顶外的货运飞船缓慢升空，机械警卫扫描通行证。第")
                    .append(round)
                    .append("轮。")
                append(tag).append(' ')
                append("阿诺把芯片藏进袖口，第九殖民区已经失联，星网只剩下一段被截断的求救信号。第")
                    .append(round)
                    .append("轮。")
                append(tag).append(' ')
                append("指挥官在全息沙盘上标出跃迁坐标，蓝色光点穿过小行星带，停在废弃矿站旁。第")
                    .append(round)
                    .append("轮。")
            }
        }
    ...
    private class TailRescueSemanticModel : V8SemanticModel {
        override fun build(
            ...
            return V8SemanticSpace(
                ...
                referenceVectors = referenceWindows.map { semanticVector(it) },
                idf = idf,
                vectorizer = ::semanticVector,
                config = config
            )
        }

        private fun semanticVector(text: String): V8SparseVector {
            return when {
                text.contains("FUTURE_RESCUE_FOREIGN") -> mapOf("book" to 0.72, "foreign" to 0.70)
                text.contains("CURRENT_FOREIGN") -> mapOf("foreign" to 1.0)
                else -> mapOf("book" to 1.0)
            }
        }
    }
